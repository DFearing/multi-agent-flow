...
export interface TopBarProps {
  ...
  /** Floor for the canvas auto-fit scale, 0 = no minimum (current default).
   *  Exposed in a small slider popover so the user can prevent the camera
   *  from zooming out too far when many agents are spread across the canvas. */
  minZoomLevel?: number
  onMinZoomLevelChange?: (next: number) => void
...
export const TopBar = memo(function TopBar({
  ...
  minZoomLevel = 0, onMinZoomLevelChange,
    ...
    >
      <div className="flex items-center gap-4 font-mono text-[12px] px-3 py-2 h-full">
        ...
        <div className="flex items-center gap-4 flex-shrink-0" style={{ color: COLORS.textMuted }}>
          ...
          />
          {onMinZoomLevelChange && (
            <MinZoomButton value={minZoomLevel} onChange={onMinZoomLevelChange} />
          )}
...
// ─── Min-zoom button + popover ──────────────────────────────────────────────
// A small slider that floors the auto-fit scale on every canvas. 0 = off
// (current default — auto-fit picks any zoom that fits the bounding box).

function MinZoomButton({ value, onChange }: { value: number; onChange: (v: number) => void }) {
  const [open, setOpen] = useState(false)
  const buttonRef = useRef<HTMLDivElement>(null)
  const popoverRef = useRef<HTMLDivElement>(null)
  const [anchorRect, setAnchorRect] = useState<{ top: number; right: number } | null>(null)
  ...
  const label = value > 0 ? `Min Zoom ${pct}%` : 'Min Zoom'
  ...
  const popoverContent = (
    ...
    >
      <div style={{ fontSize: 11, color: COLORS.textPrimary, marginBottom: 8, display: 'flex', justifyContent: 'space-between' }}>
        <span>Auto-fit floor</span>
        <span style={{ color: COLORS.textMuted }}>{value > 0 ? `${pct}%` : 'off'}</span>
      </div>
      <input
        type="range"
        min={0}
      ...
      <div style={{ marginTop: 10, fontSize: 10, color: COLORS.textMuted, lineHeight: 1.4 }}>
        Stops the canvas from zooming out below this level when fitting all
        agents. Wheel zoom is unaffected. Set to 200% to effectively lock
        zoom at the max.
      </div>
  ...
  return (
    <>
      <div ref={buttonRef}>
        <ToggleButton active={active} onClick={() => setOpen(o => !o)}>
          {label}
        </ToggleButton>
      </div>
      {open && anchorRect && typeof document !== 'undefined' && createPortal(popoverContent, document.body)}
